...
func main() {
	...
	// Get index
	twitterIndex, err := client.IndexGetSettings("twitter").Do(ctx)
	if err != nil {
		fmt.Println(err)
	}
	pp.Println("twitterIndex:", twitterIndex)

	// Create a new index.
	createIndex, err := client.CreateIndex("twitter").BodyString(setting).Do(ctx)
	if err != nil {
		// Handle error
		panic(err)
	}
	...
	// Get index mapping
	twitterMap, err := client.GetMapping().Index("twitter").Do(ctx)
	if err != nil {
		panic(err)
	}
	...
	// Put index mapping
	putMapping, err := client.PutMapping().Index("twitter").BodyString(mapping).Do(ctx)
	if err != nil {
		panic(err)
	}
